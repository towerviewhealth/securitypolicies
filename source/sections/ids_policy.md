...
In order to preserve the integrity of data that TowerView Health stores, processes, or transmits for Customers, TowerView Health implements strong intrusion detection tools and policies to proactively track and retroactively investigate unauthorized access. TowerView Health currently utilizes [OSSEC](http://www.ossec.net/) to track file system integrity, monitor log data, and detect rootkit access.
...
* TowerView Health firewalls monitor all incoming traffic to detect potential denial of service attacks. Suspected attack sources are blocked automatically. Additionally, our hosting provider actively monitors its network to detect denial of services attacks.
...
* TowerView Health utilizes redundant firewall on network perimeters.
* Static IP addresses are used for TowerView Health servers.
